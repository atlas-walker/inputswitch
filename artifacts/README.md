# InputSwitch — build lista para usar (v0.2.0, arm64)
...
- `InputSwitch-0.2.0-arm64.zip` — la app empaquetada.
- `InputSwitch-0.2.0-arm64.zip.sha256` — suma de verificación.
- `build-environment.txt` — toolchain con el que se compiló.

## Cómo correrla en el otro MacBook

1. Descarga `InputSwitch-0.2.0-arm64.zip` desde GitHub (botón **Download raw**
   o clona el repo).
2. En Terminal, dentro de la carpeta donde quedó el ZIP:

   ```sh
   # (opcional) verificar integridad
   shasum -a 256 -c InputSwitch-0.2.0-arm64.zip.sha256

   # descomprimir
   unzip InputSwitch-0.2.0-arm64.zip
